...
- 请求/响应头和请求/响应体查看，JSON 自动格式化与语法高亮；br / gzip / deflate / zstd 压缩的传输内容自动解压展示（并标注「已解压」）
...
npx httptap config   # 用浏览器直接打开配置窗口
npx httptap restart  # 重启后台服务（系统代理接管中会保持接管）
```

Web 界面右上角的「设置」按钮（或 `httptap config`）打开配置窗口，可修改：

- **代理端口 / 界面端口**：保存后服务自动重启，页面自动跳转到新地址；系统代理接管中会一并重指到新端口
- **暴露到局域网**：默认仅本机（127.0.0.1）；开启后绑 0.0.0.0，局域网设备可使用本机代理、查看面板
- **仅监控进程**：下拉多选，自动分析系统当前网络活跃进程（按连接数降序取前 10）；已选但不活跃的进程会保留在顶部，也可手动添加任意进程名。留空监控全部，匹配规则为忽略大小写和 `.exe` 后缀的子串匹配

配置保存在 `~/.httptap/config.json`，命令行 `--proxy-port` / `--ui-port` 优先级高于配置文件。
